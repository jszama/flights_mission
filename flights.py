...
def handle_func_call(func_name:str,func_args:dict):
            function_params = {}
...
def handle_response(response):  # Pass chat history
    st.write(response)
    output = []
    ...
    if part.function_call.args:
        ...
        while True:
            results = handle_func_call(func_name=function_name,func_args=function_args)
            ...
            if results != "":
             ...
             analysis = analyze_response(intermediate_response)
             if analysis==False:
...
def llm_function(chat: ChatSession, query):
    ...
    output = handle_response(response)  # Pass chat history
    print(output)
